...
public:
    ...
    void validate()
    {
        ...
        else
        {
            ...
            if (inputType == CAMERA)
            {
                inputCapture.open(cameraID, CAP_V4L);
                if (imageWidth > 0 && imageHeight > 0)
                {
                    inputCapture.set(CAP_PROP_FOURCC, VideoWriter::fourcc('M', 'J', 'P', 'G'));
                    inputCapture.set(CAP_PROP_FRAME_WIDTH, imageWidth);
                    inputCapture.set(CAP_PROP_FRAME_HEIGHT, imageHeight);
                }
            }
...
public:
    ...
    bool fixK5;
    // Image width in case of a camera as input
    int imageWidth;
    // Image height in case of a camera as input
    int imageHeight;
